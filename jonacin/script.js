...
var button_foodRandom=document.querySelector("#foodRandom")
...
addEventListener("click", function foodRandom(){
    if   (!searchFood.value){
        noBlankValue()
    } else if (searchFood.value){
        errorMessage.textContent ="";
    fetch('https://api.edamam.com/api/recipes/v2?type=public&q='+searchFood.value+'&app_id=8753c07f&app_key=25bd1618a120d76adb83aa22d43f1e73')
   ...
   cuisine_type.innerHTML  = " <b>Cuisine Type: </b>" + data.hits[index].recipe.cuisineType;
   cuisine_type.style.background = "white";


   healthLabels.style.display = "block";
   healthLabels.innerHTML  =  " <b>Health Label: </b> " +data.hits[index].recipe.healthLabels;
   healthLabels.style.background = "white";


   dietLabels.style.display = "block";
   dietLabels.innerHTML  = " <b>Diet Label: </b>" + data.hits[index].recipe.dietLabels;
   dietLabels.style.background = "white";
  
  
   ingredientLines.style.display = "block";
   ingredientLines.innerHTML =  "<b>Ingredient Lines: </b> " +data.hits[index].recipe.ingredientLines;
   ingredientLines.style.background = "white";


  
   calories.style.display = "block";
   calories.innerHTML  ="<b>Calories: </b> " + data.hits[index].recipe.calories;
